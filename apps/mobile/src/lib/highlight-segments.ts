...
export function highlightSegments(
  ...
  for (const span of [...highlights].sort((a, b) => a.start - b.start)) {
    if (normalized.slice(span.start, span.end) !== span.text) continue;
    const start = rawIndex[span.start]!;
    const end = rawIndex[span.end - 1]! + 1;
    if (start < cursor) continue;
    if (start > cursor) segments.push({ text: raw.slice(cursor, start), field: null });
